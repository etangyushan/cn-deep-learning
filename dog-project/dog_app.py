...
# # 加载彩色（通道顺序为BGR）图像
# testimg = cv2.imread(human_files[2])
# # 将BGR图像转变为RGB图像以打印
# cv_rgb = cv2.cvtColor(testimg, cv2.COLOR_BGR2RGB)
# # 将BGR图像进行灰度处理
...
def get_human_count(files_short):
    return np.mean([face_detector(human) for human in files_short])

human_count = get_human_count(human_files_short)
print(human_count)
dog_count = get_human_count(dog_files_short)
print(dog_count)


# - `human_files` 的前100张图像中，能够检测到**人脸**的图像占:99%
# - `dog_files` 的前100张图像中，能够检测到**人脸**的图像占:11%
...
# __回答:__
# 
# 在实际使用中不合理，例如安防中的人脸识别，高清摄像头的数量很少，摄像头和人的距离也比较远，加上人是活动的，很难捕捉到高清晰度的图片。
# 
# 可以捕获多张照片进行处理，提取其中较清晰的部分特征进行对比。对图片进行锐化处理，增加面部轮廓的可见性，提高检测准确率
# 
...
def get_dog_detector_count(files_short):
    return np.mean([dog_detector(animal) for animal in files_short])

humancount = get_dog_detector_count(human_files_short)
print (humancount)
dogcount = get_dog_detector_count(dog_files_short)
print (dogcount)


# - `human_files_short`中图像检测到狗狗的百分比:1%
...
# 采用三层卷积，具体步骤：{卷积层-》激活层-》池化层}*4-》全局平均池化层-》全连接层
# 
# - 卷积层：通过过滤器提取图片特征
# - 激活层：ReLU 增加非线性计算使得数据接近真实世界
# - 池化层：将图片缩小
# - 展开层：将像素展开
# - 全连接层：像素与标签的关系
# 
...
# 添加展开层，全连接层之前需要先将图片的像素展开
# model.add(Flatten())
#添加第1个全连接层，133 表示神经元的个数
model.add(Dense(133, activation='relu'))
model.add(Dropout(0.5))

# 添加第2个全连接层
# 10 表示神经元的个数，但是由于本层为CNNs 架构的最后一层（即输出层）
# 所以，此处的数值只能为10，对应0-9个数分类
# softmax 是非线性函数，输出的结果为最初输入的图片，属于每种类别的概率
...
def split_featuresdata(file_npz):
    bottleneck_features = np.load(file_npz)
...
def get_mymodel(mytrain):
    mymodel = Sequential()
    mymodel.add(GlobalAveragePooling2D(input_shape=mytrain.shape[1:]))
...
def train_model(model, file_hdf5, train, valid):
    checkpointer = ModelCheckpoint(filepath=file_hdf5, verbose=1, save_best_only=True)
...
def get_testaccuracy(model, test):
    # 获取测试数据集中每一个图像所预测的狗品种的index
...
def predict_human_dog(img_path):
    ...
    if (dog_detector(img_path) == True):
        ...
        print ('Hello human!')
        show_pic(img_path)
        dog_like = predict_breed(img_path)
        print ('You like ',dog_like)
...
print(human_files[1])
predict_human_dog(human_files[1])
...
# print(veritydata)
verity_files = np.array(veritydata['filenames'])
print(verity_files)
# verity_targets = np_utils.to_categorical(np.array(veritydata['target']), 133)
...
# 鱼
predict_human_dog(verity_files[0])
print('=================================')

# 狗 
predict_human_dog(verity_files[1])
print('=================================')

# 人类
predict_human_dog(verity_files[2])
print('=================================')


# 狗
predict_human_dog(verity_files[3])
print('=================================')


# 人类
predict_human_dog(verity_files[4])
print('=================================')


# 鸟
predict_human_dog(verity_files[5])
print('=================================')
